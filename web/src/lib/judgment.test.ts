...
describe("エリア内移動テスト（勤務地エリア内の複数地点）→ Trip 0件", () => {
  it("勤務地中心から100m以内の複数地点はすべて WORK 扱いで Trip 生成なし", () => {
    const date = "2026-04-13";
    // 全て勤務地半径100m 以内（0.0008° ≈ 89m @ 35.7°N）
    const nearby1 = { lat: WORK.lat + 0.0005, lng: WORK.lng }; // ~55m
    const nearby2 = { lat: WORK.lat, lng: WORK.lng + 0.0008 }; // ~73m
