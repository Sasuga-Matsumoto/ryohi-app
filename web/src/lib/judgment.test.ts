...
describe("休日テスト（祝日に外出）", () => {
  it("祝日 + include_holidays=false なら Trip 生成なし", () => {
    ...
    const result = judgeTrip({ date, stays, setting: baseSetting });
    expect(result.trip).toBeNull();
    expect(result.skipReason).toBe("holiday_excluded");
  });

  it("祝日 + include_holidays=true なら通常判定し Trip 生成", () => {
  ...
  it("土曜日 + include_weekends=false なら Trip 生成なし", () => {
